...
ROWS = [
    ("host",),
    ("kv", "Role", "Frontend Developer | DSA Learner"),
    ("kv", "Learn", "CODING GITA"),
    ("kv", "Doing", "Frontend + Backend + DSA"),
    ("kv", "Intrs", "Coding, Cricket, Design"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Front", "HTML, CSS, JS, React"),
    ("kv", "Back", "Node.js, Express, MongoDB"),
    ("kv", "Langs", "C, C++"),
    ("kv", "Tools", "Git, Postman, Figma"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Open to Collaborations & Learning"),
    ("bul", "Fun Fact: I think I am funny :)"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">PriyanshCG@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">PriyanshCG</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{KEY_X+136}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
